...
def split_text(page_map):
    ...
    while start + SECTION_OVERLAP < length:
        last_word = -1
        end = start + MAX_SECTION_LENGTH # end = planned max lenth

        if end > length:
            end = length # Krótkie sekcje pod limitem są przejmowane naturalnie
        else:
            # Try to find the end of the sentence
            while end < length and (end - start - MAX_SECTION_LENGTH) < SENTENCE_SEARCH_LIMIT and all_text[end] not in SENTENCE_ENDINGS: #ile zdefiniwowaliśmy do wykorzystania tokenów w sekcji - max section length < SENTENCE_SEARCH_LIMIT 
    ...
    if args.index not in index_client.list_index_names():
        index = SearchIndex(
            ...
            fields=[
                ...
                SearchableField(name="content", type="Edm.String", analyzer_name="pl.microsoft"),
                SearchField(name="embedding", type=SearchFieldDataType.Collection(SearchFieldDataType.Single),
                            hidden=False, searchable=True, filterable=False, sortable=False, facetable=False,
                            vector_search_dimensions=1536, vector_search_configuration="default"), ##### spr
