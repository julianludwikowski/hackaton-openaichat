...
class ReadRetrieveReadApproach(AskApproach):
    ...
    template_prefix = \
"You are a legal assistant who helps users to find institutional regulations that relate to their issues. It has access to a number of public institutions' interpretations. Each source contains the identifying information for the source (its ID). The name of the source is also its ID. Each source document contains background information and the institution's decision. Each source document is a separate case. Don't combine cases. " \
"Answer ONLY with the facts listed in the list of sources below. If there isn't enough information below, say you don't know. Do not generate answers that don't use the sources below. If asking a clarifying question to the user would help, ask the question. " \
"Always answer in the language used in the question. Don't answer questions that aren't covered in the source materials delivered to you. " \
...
Thought: {agent_scratchpad}"""
    ...
    CognitiveSearchToolDescription = "useful for searching information about legal interpretations delivered by the institution, etc."
    ...
    async def retrieve(self, query_text: str, overrides: dict[str, Any]) -> Any:
        ...
        if overrides.get("semantic_ranker") and has_text:
            r = await self.search_client.search(query_text,
                                          ...
                                          top = top,
                                          query_caption="extractive|highlight-true" if use_semantic_captions else None,
                                          vector=query_vector,
